<script>
    ...
    const installTempCreate = () => {
        if (needInstallTemp) {
            for (let i = 0; i < socketData.software.length; i++) {
                installTemp[socketData.software[i].id] = socketData.software[i].installed
            }
        }
    ...
    function checkButton() {
        ...
        for (const i of socketData.software) {
            if (i.installed != installTemp[i.id]) {
                if (uninstall !== undefined && uninstall !== !installTemp[i.id]) {
                    alert("ERROR: cannot install and uninstall at the same time")
    ...
    function sendSoftware() {
        socket.send(JSON.stringify({do: uninstall == true ? "uninstall" : "install", args: installArray}))
        running = true
    }

    $: socketData.software && installTempCreate()

    $: socketData.software && installTemp && (
        checkButton(),
        getNameList()
    )
</script>
...
<main>
                ...
                <tr class="mt-32 even:bg-white odd:bg-gray-200 border-t-2 border-gray-300 border-opacity-50">
                    ...
                    <td class="p-2">
                        {#if software.docs.substr(0, 5) == "https"}
                            <a href={software.docs} class="text-blue-500 underline" target="_blank">
                                {software.docs}
                            </a>
                        {:else}
                            {software.docs}
    ...
    <div class="flex justify-center my-2">
        <button on:click={sendSoftware} class="rounded border {uninstall == true ? "bg-red-500 border-red-600 hover:bg-red-700" : "bg-green-500 border-green-600 hover:bg-green-700"} p-2 disabled:opacity-50" disabled={installArray.length == 0}>
            {#if running}
